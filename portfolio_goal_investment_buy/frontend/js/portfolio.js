...
async function loadPortfolioData() {
    ...
    try {
        ...
        // Load smart buy recommendations (non-blocking)
        loadSmartBuy();
    } catch (error) {
    ...
    try {
        ...
        listEl.innerHTML = `
            <div class="smart-buy-empty">
                ...
                <p>Unable to fetch recommendations right now. Check market data connection.</p>
            </div>`;
    } finally {
...
function renderSmartBuyRecommendations(recs) {
    ...
    if (!recs || recs.length === 0) {
        listEl.innerHTML = `
            <div class="smart-buy-empty">
                <span class="smart-buy-empty-icon">✅</span>
                <p>No significant dips detected in the watchlist right now. The market looks stable — no action needed.</p>
            </div>`;
        return;
    }
    ...
    listEl.innerHTML = recs.map(r => {
        ...
        const convClass = r.conviction === 'STRONG' ? 'conviction-strong'
            : r.conviction === 'MODERATE' ? 'conviction-moderate'
                : 'conviction-watch';
        const convEmoji = r.conviction === 'STRONG' ? '🔥'
