...
function renderAllocationOverview(allocation, analysis) {
    ...
    // Use backend target weights (from rebalancing suggestions) when available,
    // otherwise fall back to equal weight
    const rebalancing = analysisData?.rebalancing;
    const riskKey = rebalancing?.risk_preference || 'moderate';
        ...
        window._currentAllocChart = new Chart(ctx1, {
            ...
            data: {
                ...
                datasets: [{
                    ...
                    backgroundColor: CHART_COLORS.slice(0, allocation.length),
                    borderColor: '#0f0f1a',
                    borderWidth: 2,
                }]
            },
    ...
    // Target allocation donut (risk-aware)
        ...
        window._targetAllocChart = new Chart(ctx2, {
            ...
            data: {
                ...
                datasets: [{
                    ...
                    backgroundColor: CHART_COLORS.slice(0, targetAllocation.length),
                    borderColor: '#0f0f1a',
                    borderWidth: 2,
                }]
            },
...
function renderDriftDetails(rebalancing) {
    ...
    // Use the risk-aware target weight from backend
    const maxSingle = rebalancing.max_single_stock_weight ?? (100 / allocation.length);
    const numHoldings = allocation.length;
    const rawEqual = 100.0 / numHoldings;
...
function renderAdjustmentSuggestions(rebalancing) {
    ...
    // Show strategy description below badge if available
    ...
    if (suggestions.length === 0) {
        tbody.innerHTML = `
            <tr class="empty-row">
                <td colspan="7">Your portfolio is well-balanced. No adjustments needed. 🎉</td>
            </tr>
        `;
        return;
    }
