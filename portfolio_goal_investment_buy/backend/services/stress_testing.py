...
class StressTestingService:
    ...
    def run_stress_test(db: Session, goal_id: int) -> Dict:
        ...
        for scenario_id, scenario in StressTestingService.SCENARIOS.items():
            ...
            # Estimate additional days needed using logarithmic formula:
            # days = log(target / stressed) / log(1 + daily_growth)
            if days_remaining > 0 and current_value > 0:
                try:
                    daily_growth = (target_value / current_value) ** (1 / days_remaining) - 1
                    if daily_growth > 1e-10 and stressed_value > 0 and target_value > stressed_value:
                        import math
                        new_days = int(math.log(target_value / stressed_value) / math.log(1 + daily_growth))
