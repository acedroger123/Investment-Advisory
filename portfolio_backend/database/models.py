"""
SQLAlchemy ORM models for the Portfolio Analysis System.
"""
...
class User(Base):
    ...
    pg_user_id = Column(Integer, nullable=True)  # Link to PostgreSQL newusers.id
...
class Goal(Base):
    ...
    __tablename__ = "pa_goals"
    
    id = Column(Integer, primary_key=True, index=True)
    user_id = Column(Integer, ForeignKey("pa_users.id"), nullable=False)
...
class Transaction(Base):
    ...
    __tablename__ = "pa_transactions"
    
    id = Column(Integer, primary_key=True, index=True)
    goal_id = Column(Integer, ForeignKey("pa_goals.id"), nullable=False)
